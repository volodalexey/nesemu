...
export class Mapper005 extends Mapper {
  ...
  constructor(options: MapperOptions) {
        ...
        switch (flag) {
          ...
          case 3:
            this.ioMap.set(adr, (adr, value): any => {
              if (value == null) return this.readIo(adr)
    ...
    for (let adr = 0x5c00; adr <= 0x5fff; ++adr) {
      this.ioMap.set(adr, (adr, value): any => {
        const i = adr - 0x5c00
        if (value == null) return this.exram[i]
        else this.exram[i] = value
      })
    }
  ...
  public reset(): void {
    ...
    for (let i = 0; i < 4; ++i) this.options.setPrgBank(i, this.maxPrg)

    for (let i = 0; i < 8; ++i) this.options.setChrBankOffset(i, i)
  }
  ...
  public load(saveData: any): void {
    super.load(saveData)
    if (saveData.prgMode != null) this.prgMode = saveData.prgMode
    if (saveData.prgBanks != null) this.prgBanks = new Uint8Array(saveData.prgBanks)
    if (saveData.chrMode != null) this.chrMode = saveData.chrMode
    if (saveData.chrBanks != null) this.chrBanks = new Uint16Array(saveData.chrBanks)
    if (saveData.upperChrBit != null) this.upperChrBit = saveData.upperChrBit
    if (saveData.abMode != null) this.abMode = saveData.abMode
    if (saveData.exram != null) this.exram = Util.convertBase64StringToUint8Array(saveData.exram)
    if (saveData.muls != null) this.muls = new Uint8Array(saveData.muls)
  ...
  public onHblank(hcount: number): void {
    ...
    this.ppuInFrame =
      hcount < VBlank.NMI &&
      (regs[PpuReg.MASK] & (PpuMaskBit.SHOW_SPRITE | PpuMaskBit.SHOW_BG)) !== 0
  ...
  private writeIo(adr: Address, value: Byte): void {
    switch (adr) {
      ...
      case 0x5205:
      case 0x5206: // Unsigned 8x8 to 16 Multiplier
        ...
        case 0:
          {
            ...
            for (let i = 0; i < 4; ++i) opt.setChrBankOffset(i, vb + i)
            const va = this.chrBanks[7] << 3
            for (let i = 0; i < 4; ++i) opt.setChrBankOffset(i + 4, va + i)
          }
          break
        case 1:
          {
            const vb = this.chrBanks[11] << 2
            for (let j = 0; j < 4; ++j) opt.setChrBankOffset(j, vb + j)
            const va = this.chrBanks[7] << 2
            for (let j = 0; j < 4; ++j) opt.setChrBankOffset(j + 4, va + j)
          }
          break
        case 2:
          for (let i = 0; i < 8; i += 4) {
            const vb = this.chrBanks[9] << 1
            for (let j = 0; j < 2; ++j) opt.setChrBankOffset(i + j, vb + j)
            const va = this.chrBanks[5] << 1
            for (let j = 0; j < 2; ++j) opt.setChrBankOffset(i + 4 + j, va + j)
          }
          break
        case 3:
          for (let i = 0; i < 4; ++i) opt.setChrBankOffset(i, this.chrBanks[i + 8])
          for (let i = 0; i < 4; ++i) opt.setChrBankOffset(i + 4, this.chrBanks[i + 4])
          break
      }
    } else {
      switch (this.chrMode) {
        case 0:
          {
            ...
            for (let i = 0; i < 4; ++i) opt.setChrBankOffset(i, va + i)
            const vb = this.chrBanks[11] << 3
            for (let i = 0; i < 4; ++i) opt.setChrBankOffset(i + 4, vb + i)
          }
          break
        case 1:
          {
            const va = this.chrBanks[3] << 2
            for (let j = 0; j < 4; ++j) opt.setChrBankOffset(j, va + j)
            const vb = this.chrBanks[11] << 2
            for (let j = 0; j < 4; ++j) opt.setChrBankOffset(j + 4, vb + j)
          }
          break
        case 2:
          for (let i = 0; i < 8; i += 4) {
            const va = this.chrBanks[1] << 1
            for (let j = 0; j < 2; ++j) opt.setChrBankOffset(i + j, va + j)
            const vb = this.chrBanks[9] << 1
            for (let j = 0; j < 2; ++j) opt.setChrBankOffset(i + 4 + j, vb + j)
          }
          break
        case 3:
          for (let i = 0; i < 4; ++i) opt.setChrBankOffset(i, this.chrBanks[i])
          for (let i = 0; i < 4; ++i) opt.setChrBankOffset(i + 4, this.chrBanks[i + 8])
          break
      }
    }
  }
