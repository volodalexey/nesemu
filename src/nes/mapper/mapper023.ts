...
class Mapper023Base extends Mapper {
  ...
  constructor(options: MapperOptions, mapping: Record<number, number>) {
    ...
    this.options.setWriteMemory(0xc000, 0xffff, (adr, value) => {
      if (0xc000 <= adr && adr <= 0xefff) {
        ...
        let newValue: number
        if (hi) newValue = (this.chrBank[bank] & ~0x1f0) | ((value & 0x1f) << 4)
        else newValue = (this.chrBank[bank] & ~0x0f) | (value & 0x0f)
  ...
  public load(saveData: any): void {
    ...
    for (let i = 0; i < 4; ++i) this.setPrgBank(i, saveData.prgBank[i])
    for (let i = 0; i < 8; ++i) this.setChrBankOffset(i, saveData.chrBank[i])
  }
