...
const kMirrorTable = [MirrorMode.SINGLE0, MirrorMode.SINGLE1, MirrorMode.VERT, MirrorMode.HORZ]
...
export class Mapper001 extends Mapper {
    ...
    this.options.setWriteMemory(0x8000, 0xffff, (adr, value) => {
      ...
      switch (adr & 0xe000) {
        ...
        case 0xa000: // CHR bank 0
          {
            ...
            if (this.chrBank[0] !== bank) this.setChrBank(0, bank)
        ...
        case 0xc000: // CHR bank 1
          {
            const bank = this.register
            if (this.chrBank[1] !== bank) this.setChrBank(1, bank)
          }
          break
  ...
  public load(saveData: any): void {
    ...
    for (let i = 0; i < 2; ++i) this.setChrBank(i, saveData.chrBank[i])
  ...
  private setChrBank(hilo: number, bank: number): void {
    if (this.chrBank4k) {
      const chr = hilo << 2
      const b = bank << 2
      for (let i = 0; i < 4; ++i) this.options.setChrBankOffset(chr + i, b + i)
    } else {
      if (hilo === 0) this.options.setChrBank(bank >> 1)
    }
