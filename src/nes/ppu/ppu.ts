...
export function getBgPat(
  chrData: Readonly<Uint8Array>,
  chridx: number,
  py: number,
...
function getPpuAddr(adr: Address, mirrorModeBit: Byte): Address {
  adr &= 0x3fff
  if (0x3000 <= adr && adr < 0x3f00) adr -= 0x1000 // Map 0x3000~3eff to 0x2000~
  ...
  if (PALET_ADR <= adr && adr <= PALET_END_ADR) {
    ...
    if ((adr & 0xfff3) === 0x3f10) adr &= 0xffef
  }
  return adr
}
...
function clearBg(
  ...
  const LINE_BYTES = NesConst.WIDTH * 4
...
export class Ppu {
  ...
  private offscreen = new Uint8Array(NesConst.WIDTH * NesConst.HEIGHT)
  ...
  public save(): PPUSaveData {
    const data: PPUSaveData = {
  ...
  public load(saveData: PPUSaveData): void {
    ...
    if (isRam) {
      ...
      for (let i = 0; i < vram.length; ++i) this.vram[i] = vram[i]
    } else {
      const vramHigh = Util.convertBase64StringToUint8Array(saveData.vramHigh)
      for (let i = 0; i < vramHigh.length; ++i) this.vram[i + 0x2000] = vramHigh[i]
    }
    ...
    this.hstatusMgr.current.set(
      HEventType.MIRROR_MODE_BIT,
      kMirrorModeBitTable[this.mirrorMode],
      -1,
    )
    for (let i = 0; i < 32; ++i)
  ...
  public setChrData(chrData: Uint8Array): void {
    ...
    if (isRam) this.chrData = this.vram
    else this.chrData = chrData
  }

  public setChrBank(value: number): void {
    const base = value << 3
    for (let i = 0; i < 8; ++i) this.setChrBankOffset(i, base + i)
  }
    ...
    switch (reg as PpuReg) {
      case PpuReg.CTRL:
        {
          ...
          this.ppuAddr =
            (this.ppuAddr & ~0x0c00) | ((value & PpuCtrlBit.BASE_NAMETABLE_ADDRESS) << 10)
          this.updateCoarseX()
      ...
      case PpuReg.SCROLL:
        ...
        if (this.latch === 0) {
          ...
          this.ppuAddr =
            (this.ppuAddr & ~0x73e0) | ((value & 0xf8) << (5 - 3)) | ((value & 0x07) << 12)
        }
  ...
  protected clearVBlank(): void {
    this.regs[PpuReg.STATUS] &= ~(
      PpuStatusBit.VBLANK |
      PpuStatusBit.SPRITE0HIT |
  ...
  public setHcount(hcount: number): void {
    ...
    switch (hcount) {
      ...
      case VBlank.NMI:
        ...
        this.setVBlank()
        if (
          (this.regs[PpuReg.CTRL] & PpuCtrlBit.VINT_ENABLE) !== 0 &&
  ...
  private renderOffscreen(
    ...
    for (let i = 0; i < n; ++i) {
      ...
      if (hline0 >= hline1) continue

      // BG
      if ((h.ppuMask & PpuMaskBit.SHOW_BG) === 0) {
        clearBg(hline0, hline1, NesConst.WIDTH, pixels, colorMask, h.palet)
      } else {
  ...
  private renderBg(
    ...
    const W = 8
    const LINE_WIDTH = NesConst.WIDTH | 0
    const vram = this.vram

    if (scrollY >= 240) scrollY = (scrollY - 256) | 0
    ...
    for (let yy = hline0; yy < hline1; ++yy) {
      ...
      for (let bbx = 0; bbx < NesConst.WIDTH / W + 1; ++bbx) {
  ...
  private renderSprite(
    ...
    const W = 8
    const LINE_WIDTH = NesConst.WIDTH
    const PALET = 0x03
    ...
    for (let h = hline0; h < hline1; ++h) {
      ...
      for (let i = 0; i < MAX_SPRITE; ++i) {
        ...
        const px1 = Math.min(NesConst.WIDTH - x, W)
  ...
  private checkSprite0Hit(hcount: number): void {
    ...
    if (
      (this.regs[PpuReg.STATUS] & PpuStatusBit.SPRITE0HIT) !== 0 ||
      (this.regs[PpuReg.MASK] & mask) !== mask
    )
      return

    const sprite0y = this.oam[OamElem.Y] + 1
    if (hcount < sprite0y || hcount >= sprite0y + 16) return
    const sprite0x = this.oam[OamElem.X]
    if (sprite0x >= 255) return

    const dy = this.getNonEmptySprite0Line()
    if (dy < 0 || hcount !== sprite0y + dy) return
  ...
  private addHevent(type: HEventType, value: number, index = -1): void {
    ...
    if (!this.hstatusMgr.current.set(type, value, index)) return

    let hcount = this.hcount + 1
    if (hcount > NesConst.HEIGHT) {
      hcount = 0
    }
  ...
  private updateCoarseX(): void {
    if (this.visible()) {
      ...
      const scrollCurr = (this.hstatusMgr.current.scrollCurr & ~0x041f) | (this.ppuAddr & 0x041f)
