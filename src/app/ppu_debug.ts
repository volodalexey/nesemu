...
import {NesConst} from '../nes/const'
...
function render8x8Chip(
  ...
  pattern: Uint16Array,
  paletHigh: number,
  paletTable: Readonly<Uint8Array>,
  clearColor: number,
  lineWidth: number,
) {
  const W = 8
...
export class PpuDebug {
  public static renderNameTable1(
    ppu: Ppu,
    pixels: Uint8ClampedArray,
    lineWidth: number,
    startX: number,
    ...
    for (let by = 0; by < NesConst.HEIGHT / W; ++by) {
      const ay = by % 30
      for (let bx = 0; bx < NesConst.WIDTH / W; ++bx) {
        const ax = bx & 31
        ...
        for (let py = 0; py < W; ++py) pattern[py] = getBgPat(chrData, chridx, py, chrBankOffset)
        render8x8Chip(
          pixels,
          (by * W + startY) * lineWidth + bx * W + startX,
          pattern,
          paletHigh,
          paletTable,
          clearColor,
  ...
  public static renderPatternTable(
    ppu: Ppu,
    pixels: Uint8ClampedArray,
    lineWidth: number,
