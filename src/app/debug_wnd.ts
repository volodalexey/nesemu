...
export class RegisterWnd extends Wnd {
  ...
  private static createPregStr(p: number): string {
    ...
    for (let i = 0; i < 8; ++i) ss[i] = (p & (0x80 >> i)) !== 0 ? chrs[i] : '.'
    return ss.join('')
  }
...
export class RamWnd extends Wnd {
  ...
  private updateStatus(): void {
    ...
    for (let i = 0; i < RamWnd.RAM_SIZE; ++i) curr[i] = bus.read8(i)
    this.memBuf1 = curr
    this.memBuf2 = prev
  }

  private render(
    startAdr: number = this.visibleStartAdr,
    endAdr: number = this.visibleEndAdr,
  ): void {
    if (this.dirty) {
      this.updateVisibleArea()
      this.dirty = false
    ...
    for (let i = startAdr | 0, end = endAdr | 0; i < end; ++i) {
      ...
      if (cell.innerText !== hh) cell.innerText = hh
  ...
  private createContent(): {
    ...
    for (let i = 0; i < RamWnd.RAM_SIZE / 16; ++i) {
      ...
      for (let j = 0; j < 16; ++j) {
        if (j > 0) line.appendChild(document.createTextNode(j === 8 ? '\u00a0\u00a0' : ' '))
...
export class TraceWnd extends Wnd {
  ...
  public updateStatus(): void {
    ...
    for (let i = inst.bytes; i < MAX_BYTES; ++i) this.bins[i] = '  '
  ...
  private putConsole(line: string): void {
    ...
    if (this.lines.length > MAX_LINE) this.lines.shift()
...
export class ControlWnd extends Wnd {
  ...
  public constructor(
    ...
    super(wndMgr, 192, 60, 'Debug Control')
  ...
  private createElement(): HTMLElement {
    ...
    line1.classList.add('flex', 'flex-row')
    ...
    line1.appendChild(this.stepBtn)
    ...
    line1.appendChild(this.runBtn)

    this.pauseBtn = document.createElement('button')
    this.pauseBtn.innerText = 'Pause'
    this.pauseBtn.dataset.var = 'pauseBtn'
    this.pauseBtn.addEventListener('click', () => {
      this.stream.triggerPause()
    })
    line1.appendChild(this.pauseBtn)
    ...
    const setText = (txt: string | number) => {
      text.nodeValue = `Speed: ${String(txt)}`
    }
    setText(GlobalSetting.emulationSpeed)
    ...
    input.min = AppSpeed.min.toString()
    input.step = AppSpeed.step.toString()
    input.max = AppSpeed.max.toString()
    input.value = GlobalSetting.emulationSpeed.toString()
