...
function parseLcsv(lcsv: string, types?: Record<string, string>): Array<object> {
  ...
  return lcsv
    ...
    .map(row => {
      const cols = row.split(',').map((col, lineNo) => {
        const i = col.indexOf(':')
        if (i <= 0) {
...
function sampleDmc(romData: Uint8Array, params: DmcParam, sampleRate: number): Float32Array {
  ...
  if (params.value != null) sampler.setDmcWrite(Reg.DIRECT_LOAD, params.value)
  ...
  const length =
    Math.ceil(
      (((params.len << 4) + 1) * 8 * kDmcRateTable[params.rate] * sampleRate) / APU_DMC_HZ,
    ) | 0
...
async function dmc2wav(
  ...
  for (const opt of opts) {
    if (count > 0) dstFileName = `${path.parse(orgDstFileName).name}.${count}.wav`
    ...
    case '.zip':
      {
        ...
        const loadedZip = await util.promisify<Uint8Array, AsyncUnzipOptions, Unzipped>(unzip)(
          buffer,
          options,
        )
